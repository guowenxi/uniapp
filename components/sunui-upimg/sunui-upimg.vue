<template>
	<view class="sunui-uploader-bd">
		<view class="sunui-uploader-files">
			<view v-for="(item, index) in upload_before_list" :key="index">
				<view class="sunui-uploader-file" :class="[item.upload_percent && item.upload_percent < 100 ? 'sunui-uploader-file-status' : '']" @click="previewImage(index,item)">
					
					<!-- step1.这里修改服务器返回字段 ！！！ -->
					<image v-show="item.upload_percent !=null " class="sunui-uploader-img" :style="upload_img_wh" :src="item.path" mode="aspectFill" />
					<image v-show="item.upload_percent ==null" class="sunui-uploader-img" :style="upload_img_wh" 
					:src="`${baseUrl}fyProHandle/file/download/${item}?rbacToken=${rbacToken}`"
					 mode="aspectFill" />
					<view v-if="!disable" class="sunui-img-removeicon right" @click.stop="removeImage(index)" v-show="upimg_move">×</view>
					<view class="sunui-loader-filecontent" v-if="item.upload_percent && item.upload_percent < 100">{{ item.upload_percent }}%</view>
				</view>
			</view>
			<view v-if="!disable" v-show="upload_len < upload_count" hover-class="sunui-uploader-hover" class="sunui-uploader-inputbox" @click="chooseImage" :style="upload_img_wh">
...
export default {
	...
	mounted(){
		if(this.defaultValue[0] && this.defaultValue[0] !=""){
			this.upload_before_list = this.defaultValue ;
		}else{
	...
	methods: {
		...
		previewImage(idx,) {
			...
			for (let i = 0, len = _self.upload_before_list.length; i < len; i++) {
				// step3.这里修改服务器返回字段 ！！！
				if(_self.upload_before_list[i].path){
					preview.push(_self.upload_before_list[i].path);
				}else{
					preview.push(`${this.baseUrl}fyProHandle/file/download/${_self.upload_before_list[i]}?rbacToken=${this.rbacToken}`);
				}
...
const upload = function(options) {
	...
	const uploadTask = uni.uploadFile({
		...
		success: function(res) {
			...
			var data ="";
			console.warn('sunui-upimg - 如发现没有获取到返回值请到源码191行修改后端返回图片路径以便正常使用插件', res.data);
			try {
	...
	uploadTask.onProgressUpdate(async function(res) {
		for (let i = 0, len = _self.upload_before_list.length; i < len; i++) {
			if(_self.upload_before_list[i]['upload_percent']){
