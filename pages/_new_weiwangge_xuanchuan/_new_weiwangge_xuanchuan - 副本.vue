<template>
	<view class="content no-padding">
		<!-- 头部轮播 -->
		<view class="carousel-section" v-if="info.fileList && info.fileList.length>0">
			<swiper class="carousel" circular @change="changeCarousel" autoplay :interval="500000">
				<swiper-item v-for="(item, index) in info.fileList" :key="index" class="carousel-item">
					<!-- @click="navToDetailPage({title: '轮播广告'})" -->
					<image style="width:99%;margin:0 auto;" mode="widthFix" :src="
			...
			<view class="swiper-dots">
				...
				<text class="num">{{ info.fileList.length }}</text>
			</view>
		</view>
		...
		<view class="new-item-box">
			...
			<view class="item-content">
				<view class="txt">
					...
					<text class="r"> <text style="color: #e32417;">{{info.peaceCoin}}积分</text></text>
				</view>
				<view class="txt">
