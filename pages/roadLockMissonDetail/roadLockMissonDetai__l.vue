<template>
	...
	<view class="content no-padding">
		<view class="map-box">
			...
			<map id="Gmap" :longitude="loc_longitude" :latitude="loc_latitude" :include-points="markers" :polyline="polyline" :markers="markers"></map>
		</view>
		<view class="bottom-box">
			<scroll-view class="detail-box" scroll-y>
				<view class="list-cell-icon">
					...
					<view class="cell-tit clamp">
						<view class="title title-1">
							...
							<text class="cell-tip">{{ info.hasPeople ? info.hasPeople : 0 }}/{{ info.maxPeople }}已领</text>
						</view>
				...
				<view class="time-select-box default-item-box">
					...
					<view class="main-box" v-for="(time,idx) in filterTimeDataList">
						...
						<tag
							class="tag-item"
							:class="item.isReceive == 0 ? 'uni-tag-red-border-disabled' : checkfilterTimeData==item.id ? 'uni-tag-red-border' : 'uni-tag-defalut-border'"
			...
			<view class="bottom-btn-box"><button class="bottom-float-button " type="red" @click="getCheckTask">领取任务</button></view>
		...
		<uni-popup ref="popup_explain">
			<view class="time-select-box default-item-box" style="height:auto;">
				...
				<view class="bottom-btn-box"><button style="margin-top:30px;" class="bottom-float-button" type="red" @click="popClose">确定</button></view>
			</view>
		</uni-popup>
...
export default {
	...
	methods: {
		...
		changeTag(item, index) {
			...
			this.checkfilterTimeData  =item.id;
 ...
 */
		...
		getCheckTask() {
			...
			debugger
			this.$http
				.post('fyTaskManage/peaceTaskApp/joinCheckTask', {
					taskId: this.taskId,
					rbacToken: this.user.rbacToken,
					// taskScheduleIds: this.checkfilterTimeData.join(","),
					taskScheduleId: this.checkfilterTimeData
				})
				.then(res => {
					uni.hideLoading();
					if (res.data.success == 1) {
