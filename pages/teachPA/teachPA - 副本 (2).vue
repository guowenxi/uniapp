...
<script>
	...
	export default {
		...
		methods:{
			...
			//监听移动
			touchmove(e,_){
				this.scrollClientY = e.touches[0].clientY-this.touch_start_num;
				this.domHeight = this._domHeight +this.scrollClientY*2;
				console.log(this.domHeight);
				if(this.domHeight <=0){
					this.domHeight = 0;
					return ;
				}
				if(this.domHeight>=this.domHeight_default){
					this.domHeight = this.domHeight_default;
					return ;
				}
