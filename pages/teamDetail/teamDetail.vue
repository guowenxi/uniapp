...
<!-- @click="selImg" -->
		...
		<view class="cover-container cover-img-box" @click="navToAndSaveItem(item,'/pages/teamDetailPeople/teamDetailPeople')">
			...
			<view class="cover-img">
				<view class="cover-img-item"  v-for="(item, index) in list.slice(0,6)">
					<!-- <view class=""></view> -->
					<view class="img-wrap">
...
<!-- 		<scroll-view scroll-y  class="scroll-bar">
		...
		<view class="bottom-side">
			<button class=" btn new-bg-red"  @click="addTeam"   :class="item.isApply == 1 ?  '' : 'red-btn' " >申请加入</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods:{
			...
			getUserByTeamId(){
				this.$http.get('fyVolunteer/fySafeTeam/getByTeamId', {
					teamId:this.id,
						rbacToken: this.user.rbacToken,
					})
					.then(res => {
