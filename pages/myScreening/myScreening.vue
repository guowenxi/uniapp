	...
	<!-- 		<pickerSelect class="pick-box" :sel-list="stateList" keyName="sub_name" @btnClick="changeSelect($event,'sub_name')"></pickerSelect> -->
		...
		<scroll-view class="history-section" scroll-y  @scrolltolower="getmoreData()">
			...
			<list-cell-item
			 ...
			 :item = "item"
			 @eventClick="navToAndSaveItem(item,'/pages/_new_screeningDetail/_new_screeningDetail')"></list-cell-item>
		</scroll-view>
	</view>
</template>
<script>
		import listCellItem from '@/components/list-cell-item.vue';
	...
	export default {
		...
		methods: {
			changepage(item){
				...
				'/pages/screeningDetail/screeningDetail' 
				)
			},
			getmoreData(){
