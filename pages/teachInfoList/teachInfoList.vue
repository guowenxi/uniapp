...
/* 					{
		...
		onLoad(){
			...
			if(this.selListItem.questionList){
				this.info = this.selListItem.questionList;
				this.info.map((item,idx)=>{
					if(item.type == 2){
		...
		methods: {
			...
			//完成
			wancheng(){
				this.$http.post('fyPeaceStudy/fySafeQuestion/insertAnswerQuestions', {
