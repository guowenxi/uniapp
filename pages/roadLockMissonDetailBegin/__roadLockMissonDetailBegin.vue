			...
			getUnreadMessageList(){
				...
				this.$http
					.get('fyTaskManage/peaceTaskApp/getUnreadMessage', {
						...
						taskId:this.info.id,
						checkStart:this.info.taskSchedule[0].checkStart,
						checkEnd:this.info.taskSchedule[0].checkEnd,
						checkDate:this.info.taskSchedule[0].checkDate,
					})
					.then(res => {
				...
				this.$http.post('fyTaskManage/peaceTaskApp/doCheckTask', {
					...
					if (res.data.success == 1) {
						...
						if(state == 2){
							...
							this.navToAndSaveItem(res.data.data,
