	...
	class="content no-padding" 
	 scroll-y 
	  ...
	  v-on:scroll.passive="onScroll"
	   >
		<view  @click="yindaoNext" v-if="yindaoState">
			<view class="yindao-bg" v-if="yindaoPage == 0 && gridType == '0'">
				<view class="mask-box mask-box-0"></view>
				<image  class="yindao-img yindao-img-0" src="/static/yindao/1.png"></image>
			</view>
			<view class="yindao-bg" v-else-if="yindaoPage == 0 && gridType != '0'">
				<view class="mask-box mask-box-1"></view>
				<image  class="yindao-img yindao-img-1" src="/static/yindao/2.png"></image>
			</view>
			<view class="yindao-bg" v-else-if="yindaoPage == 1 && gridType != '0'">
				<view class="mask-box mask-box-2"></view>
				<image  class="yindao-img yindao-img-2" src="/static/yindao/3.png"></image>
			</view>
			<view class="yindao-bg" v-else-if="yindaoPage == 2 && gridType != '0'">
				<view class="mask-box mask-box-3"></view>
				<image  class="yindao-img yindao-img-3" src="/static/yindao/4.png"></image>
			</view>
			<view class="yindao-bg" v-else-if="yindaoPage == 3 && gridType != '0'">
				<view class="mask-box mask-box-4"></view>
				<image  class="yindao-img yindao-img-4" src="/static/yindao/5.png"></image>
			</view>
		</view>
		<view class="fab-box"
		 v-if="gridType != '0'"
		...
		<view class="title-box" @click="changeTitleSearch">
			...
			<scroll-view scroll-x class="right-b" >
...
<!-- 						<view class="main-text sub">
			...
			@click="navToAndSaveItem(item,'/pages/_new_weiwangge_detail/_new_weiwangge_detail')">
				<view class="title main-text">
					<!-- <image class="icon" src="/static/_new/wwg_ID@2x.png"></image> -->
					<view class="line-box">
						<view class='line'></view>
...
<!-- 						<view class="main-text sub">
		...
		<view class="new-title-box" v-if="gridType != '0'" :class="pageState ? 'fixed-box' :  '' ">
			<view class="tab">
				<new-wuc-tab :tab-list="tabList" :tabCur.sync="TabCur" @change="tabChange"></new-wuc-tab>
			</view>
		</view>
		<view class="new-title-bar" v-if="gridType == '0'" :class="pageState ? 'fixed-box' :  '' ">
		...
		<view class="history-section icon"
		 :class="pageState ? 'fixed-box-content-44' :  '' "
		   v-if="TabCur == 0">
			<image  class="prompt_img" v-if="Number(total) === 0 ? true : false" src="/static/img/prompt_blank@2x.png"></image>
		...
		<view class="history-section icon" 
		:class="pageState ? 'fixed-box-content-88' :  '' "
		 v-else-if="TabCur == 2">
			<image v-if="Number(total) === 0 ? true : false" class="prompt_img" src="/static/img/prompt_blank@2x.png"></image>


			<new-list-cell-xxsb v-for="(item, index) in list"
			 :item="item"
			  :state="TabCur_xxsb"
			:itemClick="TabCur_xxsb != 0 ? true:  false"
			@onlongpress = "onlongpress(item,'删除')"
			@tianxie="xxsb(item, '填写')" @detail="xxsb(item,'详情')"></new-list-cell-xxsb>
	...
	export default {
		...
		data() {
			return {
				...
				yindaoState:false,
				pageState:false,
				domHeight_default:uni.upx2px(440),
				checkedText:"",
				filterText:"",
		...
		onLoad() {
			...
			this.yindaoState = uni.getStorageSync('firstLoginIn');
			// this.yindaoState = true;
			if(this.yindaoState) this.yindaoPage = 0;
			uni.setStorageSync('firstLoginIn',false);
		},
		onUnload() {
			//移除所有监听
			uni.$off();
		...
		methods: {
			...
			onlongpress(item,_item,_){
				debugger
			},
			//监听滚动
			onScroll(e,_){
				console.log(e.target.scrollTop);
				if(e.target.scrollTop>this.domHeight_default){
					this.pageState = true;
					return ;
				}
				if(e.target.scrollTop<=this.domHeight_default){
			...
			xxsb(item, type) {
				switch (type) {
					...
					//1是多次 2是单次
					if(item.reportNumber == 1){
						this.navToAndSaveItem(item, '/pages/_new_xinxishangbao_detail/_new_xinxishangbao_detail');
					}else{
						this.navToAndSaveItem(item, '/pages/_new_xinxishangbao_detail_list/_new_xinxishangbao_detail_list');
					}
						break;
					case '修改':
						item.TabCur_xxsb = this.TabCur_xxsb;
		...
		onNavigationBarButtonTap(e, _) {
			this.navTo("/pages/_new_weiwangge_map/_new_weiwangge_map");
		},
	};
</script>
<style lang="scss">
	...
	.fixed-box-content-44{
		margin-top: 88rpx;
	}
	.fixed-box-content-88{
		margin-top: 88rpx;
	}
	...
	.swiper-box-wangge {
		...
		.title {
			background-image: linear-gradient(-20deg, #fff5ec 0%, #ffecdf 67%, #ffe2d2 100%);
			height: 62upx;
			padding: 0 20upx;
			width: auto;
			.title-text{
